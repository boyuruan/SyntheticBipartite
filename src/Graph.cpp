...
void Graph::generate_graph(enum Strategy s) {
    if (n1 > n2) {
        swap(n1, n2);
    }
    ...
    if (s == Strategy::fix) {
        ...
        while (bloomCnt != 0 && n2 != 0) {
                ...
                for (int i = 0; i < vertex_to_add.size(); i++) {
                    ...
                    if (uses[eid] == 0) {
                        ...
                        if (index != remaining_vertex.size() - 1) {
                            swap(remaining_vertex[index],
                                 remaining_vertex.back());
            ...
            if (n2 != 0)
                dis_vertex.param(
                    uniform_int_distribution<>::param_type(0, n2 - 1));
        }
    } else if (s == Strategy::step) {
        vector<int> sizes(bloomCnt, 0);
        vector<int> uses(n2, overLap);
        vector<int> remaining_vertex;
        remaining_vertex.reserve(n2);
        double f = 1.0;
        for (int i = 0; i < bloomCnt; i++) {
            sizes[i] = (int)ceil(bloomSize * f);
            if (f >= 0.3)
                f *= prob;
        }
        for (int i = 0; i < n2; i++) {
    ...
    } else if (s == Strategy::grad) {
        vector<int> sizes(bloomCnt, bloomSize);
        vector<int> uses(n2, 0);
        vector<int> remaining_vertex;
        remaining_vertex.reserve(n2);
        double f = 1.0;
        for (int i = 0; i < n2; i++) {
            uses[i] = (int)ceil(overLap * f);
            if (f >= 0.3)
                f *= prob;
        }
        for (int i = 0; i < n2; i++) {
